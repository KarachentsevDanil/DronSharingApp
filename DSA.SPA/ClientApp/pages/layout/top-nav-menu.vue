...
export default {
  ...
  async beforeMount() {
    // let residentsNavItems = (await homeService.GetResidentsNavigationItems()).data;
    // residentsNavItems.map(el => {
    //     el.display = el.firstName + ' ' + el.lastName;
    //     el.path = '/profile/' + el.residentId + '/timeline';
    //     el.iconClass = "icon-user";
    // });
    // this.navigationItems = this.navigationItems.concat(residentsNavItems);
  },
  computed: {
    ...mapGetters({
