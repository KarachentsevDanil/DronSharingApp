...
namespace RCS.BLL.Services
{
    public class ObservationService : IObservationService
    {
        ...
        public IEnumerable<ObservationDto> GetObservationsByParams(ObservationParams filterParams)
        {
            var observations = _unitOfWork.ObservationRepository.GetResidentObservations(filterParams);
            var result = AutoMapper.Mapper.Map<IEnumerable<Observation>, List<ObservationDto>>(observations);
            return result;
        }
    }
}
